...
def Encryption(x:String,shift:Int):String={
  ...
  val encrypted = x.map(y=>{if((y>=97 && y<=122)||(y>=65 && y<=90))(y+shift).toChar else y.toChar}).mkString
...
def Decryption(x:String,shift:Int):String={
  ...
  val decrypted = x.map(y=>{if((y>=97 && y<=122) ||(y>=65 && y<=90))(y-shift).toChar else y.toChar}).mkString  
